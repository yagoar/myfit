...
**Status:** Phase 5 verified. 42 of 48 landmarks reviewed in Blender (SMPL-X
v1.1 female T-pose). Per [GUARDRAILS.md §3](../GUARDRAILS.md), vertex IDs
require visual verification in Blender. The interactive review session is
the verification — see `references/smplx_landmark_review.json` for the
per-landmark audit trail (status: confirmed / corrected / mirrored / skipped).

Authoritative columns:

- `Proposed` — algorithmic first-pass from
  `scripts/propose_smplx_landmarks.py` (joint position + geometric filters).
- `Verified` — vertex ID committed after Blender review. Authoritative.

## Workflow

1. Run `python scripts/propose_smplx_landmarks.py` to generate proposed
   vertex IDs from SMPL-X joint positions.
2. Open the SMPL-X v1.1 female T-pose in Blender (via the official SMPL-X
   addon, v1.1 model + DLC).
3. Run `scripts/blender_landmark_review.py` in Blender's Scripting tab.
   The N-panel "Landmarks" tab iterates left-side + unpaired landmarks;
   right-side pairs auto-mirror from confirmed left picks.
4. For each landmark: confirm, correct (click correct vertex + use selected),
   or skip. Results auto-save to `/tmp/landmark_review.json`.
5. Final verified IDs from the JSON populate the `Verified` column below.
...
| Landmark | Description | Nearest SMPL-X joint | Proposed | Verified |
|----------|-------------|----------------------|----------|----------|
